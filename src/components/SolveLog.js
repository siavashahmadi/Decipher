...
const calculateAverage = (times, size) => {
  ...
  const recentTimes = times.slice(0, size)
    .map(solve => solve.dnf ? Infinity : solve.plus2 ? solve.time + 2 : solve.time);
          ...
          return (
            <li key={solve.id} className="solve-log-item">
              <span className="solve-time">
                {solve.dnf ? 'DNF' : 
                solve.plus2 ? `${(solve.time + 2).toFixed(2)}+` : 
              ...
              <div className="solve-actions">
                ...
                <button 
                  onClick={() => onSolveUpdate({ ...solve, plus2: !solve.plus2 })}
                  className={`plus2-button ${solve.plus2 ? 'active' : ''}`}
                >
                  +2
                </button>
                <button 
